...
@pytest.mark.anyio
async def test_healthz_use_case_execute_returns_healthy():
...
@pytest.mark.anyio
async def test_healthz_use_case_readiness_returns_ok_with_api_server_dependency():
    use_case = HealthzUseCase(
        database_health_output_port=_HealthyDatabaseHealthOutputPortStub(),
    )
...
@pytest.mark.anyio
async def test_healthz_use_case_readiness_returns_not_ok_when_database_is_unhealthy():
    use_case = HealthzUseCase(
