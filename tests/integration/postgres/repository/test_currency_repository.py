...
from src.core.domain.currency import CurrencySortableField
...
from tests.integration.postgres.helpers.builders import (
    build_currency_changes as _build_currency_changes,
)
from tests.integration.postgres.helpers.builders import (
    build_new_currency as _build_new_currency,
)
...
@pytest.mark.anyio
async def test_create_currency_raises_conflict_when_iso_code_already_exists(
    postgres_session_factory: async_sessionmaker[AsyncSession],
) -> None:
    async with postgres_session_factory() as session:
        repository = SQLAlchemyCurrencyOutputAdapter(session)
